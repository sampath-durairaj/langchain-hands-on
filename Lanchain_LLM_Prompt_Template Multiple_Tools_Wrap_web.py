...
try:
    ...
    search_query ="Latest updates on GPT by OpenAI"

    print(query)
    llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0, openai_api_key=API_KEY)
    tavilla_llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0, openai_api_key=TAVILY_API_KEY)
    ...
    search_prompt = PromptTemplate.from_template("Searches the web for relevant information: {test}")
    search_chain = LLMChain(llm=tavilla_llm, prompt=search_prompt)
    search_tool = Tool(description = "Searches the web for relevant information" ,
                    name="Web Search", 
                    func=TavilySearch(max_result=3).run)
    

    
    qa_response = qa_tool.run(query)
    summary_response = summary_tool.run(summary_text)
    search_response = search_tool.run(search_query)
